...
- Outcome: implementation/remediation is a candidate for independent re-review. The source projection reserves `Не указан`, keeps `Другие` limited to overflow known sources, and presents a known literal source `Не указан` as `Источник: Не указан`. Focused local PHPUnit checks pass with 42 tests / 286 assertions across Analytics, Dashboard, UpcomingBookings, and M11A attribution coverage; changed PHP syntax, Pint, affected-production PHPStan, and `git diff --check` pass. PostgreSQL remains `EXISTS — NOT RUN LOCALLY`: `phpunit.xml` forces SQLite `:memory:` and no service is listening on `127.0.0.1:5432`. Hosted exact-SHA CI, staging, deployment, merge, owner/manual acceptance, and M12 work remain out of scope.

...
function createScenarioFixture(): ScenarioFixture {
    const php = `
        ...
        $template = app(\\App\\Modules\\Scenarios\\Application\\CreateNotificationTemplate::class)->handle($admin, [
            ...
            'name' => 'Playwright scenario template',
            'locale' => 'en',
            'purpose' => 'service',
            'is_active' => true,
            'subject' => null,
...
test('staff can configure a scenario timing and inspect delivery history', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Scenario Rules' })).toBeVisible();
    await page.goto(`/admin/scenario-rules/${fixture.ruleId}/edit`);
    await page.getByRole('spinbutton', { name: 'Delay*' }).fill('48');
    const save = page.getByRole('button', { name: /Save changes|Save/i });
    ...
    await expect(page.getByText('telegram — pending')).toBeVisible();
});

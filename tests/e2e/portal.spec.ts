...
async function assertRenderedViewportGeometry(page: Page, primarySelectors: string[], boundarySelectors: string[] = []): Promise<void> {
    const geometry = await page.evaluate(({ primarySelectors: selectors, boundarySelectors: boundaries }) => {
        const viewport = document.documentElement.clientWidth;
        const boxes = (items: string[]) => items.flatMap((selector) => Array.from(document.querySelectorAll<HTMLElement>(selector))
            .filter((element) => window.getComputedStyle(element).display !== 'none' && window.getComputedStyle(element).visibility !== 'hidden')
            .map((element) => {
        ...
        return {
            ...
            primary: boxes(selectors),
            boundaries: boxes(boundaries),
        };
    }, { primarySelectors, boundarySelectors });

    expect(geometry.scrollWidth).toBeLessThanOrEqual(geometry.viewport);
    expect(geometry.primary.length).toBeGreaterThan(0);
    for (const box of [...geometry.primary, ...geometry.boundaries]) {
        expect(box.left, `${box.selector} left edge`).toBeGreaterThanOrEqual(-1);
        expect(box.right, `${box.selector} right edge`).toBeLessThanOrEqual(geometry.viewport + 1);
    }
}

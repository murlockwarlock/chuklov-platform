...
- The `16` failures were eight unique tests duplicated across Chromium desktop and WebKit mobile: five known pre-M11D stale CRM/scenario locators and three E2E contract/fixture problems involving the Telegram Home mock, Profile strict locator, and medical encryption-key fixture. Only `tests/e2e/crm.spec.ts`, `tests/e2e/scenarios.spec.ts`, and `tests/e2e/portal.spec.ts` are changed; production behavior is unchanged.
- The resulting SHA is not yet hosted-E2E or full-CI proven and requires new exact-SHA gates. Staging remains exactly `3dc9f8b9a4038831823a687fa53abe6f481302b0`; deployment and merge are not run; M11 remains `IN_PROGRESS`; M12 remains `NOT_STARTED`; OQ-007 remains `OPEN`; the ordinary Booking AUTO/MANUAL Phase-1 gap remains open; and PR #23 remains untouched.

## 2026-08-30 — M11D Zoom PostgreSQL test-barrier remediation candidate

- This bounded test-harness candidate starts exactly at `ad12d6cb5000467b54b41edcef50c59104d993a7`; the narrow independent Filament re-review was `GO`. Hosted exact-SHA PostgreSQL concurrency run `33313698105` used `pgvector/pgvector:0.8.2-pg18-trixie` and recorded `65` passed, `3` failed, `415` assertions, `0` skips, and `142.09s`. M10 target-credential-before-provider, B2B booking-vs-SalesCall, first explicit/explicit Zoom, and Knowledge cleanup cases passed.
- Only the existing blank/explicit, existing explicit/explicit, and first blank/explicit Zoom cases failed with approximately 30-second `ProcessTimedOutException`s. Bounded PostgreSQL diagnostics showed the child dependency chains ending at the PHPUnit parent artificial barrier, so the remediation is limited to cumulative STDERR readiness observation in the test harness. Parent credential/organization barriers, real two-process `SaveB2bZoomConfiguration` execution, and the production Zoom/credential architecture remain unchanged.
- The resulting SHA is not yet hosted-PostgreSQL proven. PostgreSQL is `NOT RUN LOCALLY` because `pg_isready` is unavailable and no `127.0.0.1:5432` listener exists; hosted CI/full CI, deployment, and merge have not run for this candidate. Staging remains exactly `3dc9f8b9a4038831823a687fa53abe6f481302b0`; M11 remains `IN_PROGRESS`; M12 remains `NOT_STARTED`; OQ-007 remains `OPEN`; the ordinary Booking AUTO/MANUAL Phase-1 gap remains open; and PR #23 remains untouched.
...
- `afee6e31d68418c945971fefbf73aacca83f833a` received independent Portal owner-UX review `GO` with zero P0/P1 findings. The reviewer identified three P2 findings: `PORTAL-REG-001`, `READINESS-DRIFT-001`, and `TEST-EVIDENCE-001`.
- This resulting bounded candidate remediates those findings with one `portal.content_sections` registry independent of Telegram menu visibility, one shared tenant-scoped B2B eligible-specialist authority, and direct feature evidence for the affected content, readiness, continuation-security, Telegram, and referral paths.
- The resulting SHA is not independently re-reviewed; hosted CI was not run; the candidate was not deployed; staging remains exactly `c964f35995c52067c105ef6525237fe27762fd88`; the owner has not retested it. M11 remains `IN_PROGRESS`; M12 remains `NOT_STARTED`; OQ-007 remains `OPEN`; PR #23 remains untouched.
- CRM analytics `social` wording and the broadcast recipient-condition builder remain intentionally deferred to a separate CRM UX package.
...
- This bounded remediation rejects username/password userinfo and any query or fragment on the canonical Mini App origin, adds focused regression coverage, and removes the volatile M11D SHA from the roadmap gate wording. The resulting SHA has not received independent re-review, hosted CI, deployment, or owner retest; staging remains on `782f8a9f07de5cda7466225da54b5a5632b3c6f5`.
- `REQ-B2B-001` remains `CANDIDATE`; M11 remains `IN_PROGRESS`; OQ-007 remains `OPEN`; OQ-015 remains unresolved; M12 remains `NOT_STARTED`; PR #23 remains untouched.
...
- M11D remains `NOT owner-accepted`; `REQ-B2B-001` remains `CANDIDATE`; M11 remains `IN_PROGRESS`. The candidate described here later received independent review; the URL-001 follow-up above still requires fresh exact-SHA hosted CI, redeployment, owner retest, and independent re-review. OQ-007 remains OPEN, OQ-015 remains unresolved, M12 remains `NOT_STARTED`, and PR #23 remains untouched.

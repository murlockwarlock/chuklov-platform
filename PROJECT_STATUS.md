...
## 2026-08-25 — M11A remediation implementation candidate
...
- M11 is `IN_PROGRESS`. The active M11A remediation branch is `codex/m11a-acquisition-referrals-feedback`, starting at requested SHA `888dea369cfe4cdaad284db8d33e4088ee8562e5`. This batch implements only the unblocked attribution, product-neutral referral relationship, neutral Finance evidence, and NPS/feedback remediation.
- OQ-007 remains OPEN for per-product/program qualification and all reward economics/lifecycle rules: amount, percentage, points, currency, earning eligibility, redemption, expiry, refund reversal, and cash-out. M8 remains `BLOCKED` by OQ-015; no 9-systems/MSQ content is fabricated.
- M11A now has durable acquisition proof across the identity commit/finalization crash boundary; automatic referral-link and authorized manual-CRM relationship establishment; bounded provenance/audit; a module-neutral post-commit settlement event; independent idempotent neutral finance-evidence consumption; keyed feedback request fingerprints; restrictive consumed-attribution ownership; and PostgreSQL composite semantic constraints. It does not add Broadcast Engine, segmentation, analytics, reward controls, subscriptions, courses, or bot commerce.
- Local PostgreSQL/Docker was unavailable: the M11A database/concurrency tests exist and are wired into the PostgreSQL Make targets but are `EXISTS — NOT RUN LOCALLY`; no PostgreSQL pass, hosted CI, deployment, merge, or independent acceptance is claimed. The final focused candidate batch passed 110 tests / 696 assertions with 9 PostgreSQL-only skips. The M11A Unit/Feature/auth subset passed 26 tests / 193 assertions. Pint, PHP syntax, Larastan (`errors=0`), ESLint, `vue-tsc`, Vite build, Composer audit, npm audit, and `git diff --check` passed. `make privacy` and the repository Docker-backed secret scan could not complete because Docker/Colima is unavailable locally. A separate broader CRM UX batch remains one unrelated pre-existing Finance currency-format assertion failure; the affected M11A/auth/Portal/Telegram/Finance batch is green.

...
- Focused local evidence: `PhaseOneBookingLocationsTest` 8 / 59 and `MilestoneFiveScenarioTest` 26 / 157 pass; PHPStan, Pint, frontend lint/typecheck/build, PHP syntax, and `git diff --check` are passing. Hosted exact-SHA CI attempt 3 is green, including PostgreSQL integration, and the exact candidate passes `./scripts/staging-smoke.sh`; focused Phase 1 browser scenarios pass while 6 existing baseline E2E failures remain. Manual staging UI verification and owner acceptance are pending; production is untouched.

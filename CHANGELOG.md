...
- Updated the stale DeepSeek provider catalog for the official experimental `deepseek-v4-flash-vision-exp` model. The installed `laravel/ai` v0.10.3 Chat Completions adapter already transports image attachments, while text-only DeepSeek releases remain fail-closed for image workflows; the exact Vision model remains `preview` rather than a production default.
- Focused local workflow/evaluation/CRM checks, Pint, and scoped PHPStan pass. PostgreSQL workflow/concurrency checks pass; exact-SHA hosted CI run `34019825050` is green for candidate `7656e102b76b19c06336eeb24a23255b32688b05`; and the same SHA was deployed to staging with `./scripts/staging-smoke.sh` passing. Real-provider staging verification and the explicitly configured real-provider browser scenario were not run because no approved staging provider configuration was present. Production is not deployed, no merge was performed, and owner acceptance remains pending.
...
- Focused local B2B coverage passes `117` tests / `701` assertions and adjacent B2B/Scheduling coverage passes `20` / `79`; Pint, scoped Larastan, and `git diff --check` pass. PostgreSQL integration is `EXISTS — NOT RUN LOCALLY` because the local PHPUnit configuration skips the 9 PostgreSQL-only tests. Hosted exact-SHA PostgreSQL/concurrency run `33375868688`, full CI run `33376085585`, and E2E run `33376458122` all pass on implementation candidate SHA `1a328dda3136f93735d833fc04fb1a9a08ba79d2`; recorded staging remains `3dc9f8b9a4038831823a687fa53abe6f481302b0`; no deployment, merge, PR readiness, attachment, PR #23, or M12 work is included.
...
- Replaced the CRM E2E FilePond global-registry synchronization with the visible Filament upload label associated with `Перетащите файлы или выберите` and the Playwright file chooser flow. The success path now proves the visible selected filename, one uploaded row, the authorized `Открыть` action, the signed response headers, and the downloaded PDF bytes. Hosted E2E runs `33326965122` at `cb4c15367714f90607ff4022c08b94d4de661b69`, `33327304090` at `6806041c4f0ca38872489276937252c64d5fc3f3`, and `33327695887` at `14960b72cf2d346a098fa987939ae0c3b3f5fa3f` each recorded `42` passed, `2` failed, and `0` skipped while resolving the rendered FilePond control and selected-file state; the visible-label and selected-file-group corrections are in `ff3fb65` and `cb146a6`. The two consecutive qualifying hosted E2E runs `33328068795` and `33328319660` each recorded `44` passed, `0` failed, and `0` skipped across 22 Chromium desktop and 22 WebKit mobile tests; hosted full CI run `33328557362` passed Quality, Integration foundation, Integration RAG, Integration concurrency, Privacy and secret scan, and Docker build/runtime health on the same implementation candidate. Deployment, merge, PR readiness, and M12 remain unchanged.

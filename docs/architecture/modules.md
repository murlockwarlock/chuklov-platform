...
- Broadcasts: organization-scoped marketing campaigns, typed allowlisted segment queries, immutable audience/template snapshots, centralized eligibility, bounded batch/recipient claims, delivery evidence, and CRM orchestration. It depends on Identity/Consent, Scheduling, Surveys, Attribution/Referrals, Scenarios templates, and the Channels outbound boundary; it does not own those source records or call a provider adapter directly.

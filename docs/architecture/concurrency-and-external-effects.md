...
Client Companion processing snapshots the same bounded M10 whole-run execution deadline used by `AiRun`, and its processing lease lasts through that deadline plus the existing bounded lease grace. The engine receives the turn snapshot, so provider attempts, tools, failover, AiRun recovery, and Companion terminal fencing share one absolute execution window. Renewal is not used: reset, handoff, cancellation, token replacement, lease expiry, or the hard deadline immediately prevents the old worker from publishing, while a worker that is genuinely stale becomes reclaimable after the bounded grace.

Legacy Companion adoption and live transport resolution use one narrow lock hierarchy: organization/client ownership serialization, binding identity, binding row, and relevant conversation rows in ascending primary-key order. Binding uniqueness remains the final database guard; bounded transaction retries handle a unique-constraint race but do not replace coherent lock ordering.

...
Real M2/M3 instants use PostgreSQL timezone-aware timestamp columns. Date-only future values use `DATE`; display formats never enter the model. Service prices are non-floating-point integer minor units paired with an explicit ISO-like three-letter currency. Specialist timezone context is optional and validated as an IANA identifier; future scheduling may resolve the organization timezone when it is absent.

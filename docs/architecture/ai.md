...
The Client Companion is a client-facing workflow on top of the M10 control plane, not a provider or chatbot subsystem. Every turn executes through `AiWorkflowEngine` with `AiCapability::ClientCompanion`, immutable organization-managed prompt/model configuration, existing budget/fallback/provenance, the existing `search_knowledge_base` tool, protected trace, and the typed structured result contract. The active `AiPromptVersion.system_prompt` is the sole source of model instruction text; deterministic safety, authorization, handoff, and output controls remain enforced by the application, and retrieved Knowledge remains untrusted data.

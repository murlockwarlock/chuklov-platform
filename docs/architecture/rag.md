...
When an active ready revision has no ready run compatible with the current embedding configuration, CRM offers `Переобработать для поиска`. This requests the existing ingestion job without changing the immutable revision or its ready status; a failed re-embedding run leaves the material ready and can be requested again. Failed-content retry remains a separate latest-revision operation. If manual retry dispatch fails after the failed-to-pending transition, the application compensates only while the same source/revision is still active, latest, and pending, restoring `failed` without overwriting authoritative worker progress.

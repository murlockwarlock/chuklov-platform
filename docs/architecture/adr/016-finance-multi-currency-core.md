...
- Keep open-balance valuation separate from payment history: settlement entries are summed in settlement currency, while the remaining open amount is converted with the obligation's immutable conversion snapshot. Payment entries retain their own historical rate snapshots and are never subtracted from the obligation's base/display snapshot at a later rate.
- Make the finance upgrade deterministic and organization-scoped: bootstrap a missing configuration only from existing priced Service currencies, configure a single currency as force-single, leave organizations without priced Services for explicit owner setup, and fail the guarded upgrade when multiple priced currencies make a base choice ambiguous. Saving configuration validates existing currencies and all required directed rates in one transaction.

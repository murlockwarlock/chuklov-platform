...
- Kept ordinary client passwordless authentication separate. `REQ-SEC-007` is `CANDIDATE` pending the cumulative staging/browser owner-acceptance cycle; production remains closed.
...
- Independent remediation re-review returned GO FOR M6 CLOSEOUT. `REQ-CURRENCY-001`–`REQ-CURRENCY-003` and `REQ-PAYMENT-001`–`REQ-PAYMENT-005` are accepted for M6. `REQ-PAYMENT-006` and OQ-005 remain future/open for M13; no real payment provider, checkout, provider signature algorithm, or refund API was added.

...
| OQ-015 | REQ-SURVEY-002 | Provide the approved full “9 systems” and MSQ questionnaire sources, including exact questions, answer options, scoring rules, thresholds, tags, result text, locale variants, and provenance/licensing constraints. Recovered Appendix 1 is the onboarding specification and recovered Appendix 2 is the specialized-agent prompt specification; neither contains the complete 9 systems/MSQ bodies or scoring. The separate questionnaire source remains absent. | Importing/activating the required initial definitions and closing M8; the generic engine remains unblocked |
...
Only an authoritative same-organization ReferralRelationship, the matching referred Client, and authoritative Finance settlement/commercial evidence can qualify a reward. Organic source text, Telegram username, phone/name similarity, and marketing attribution alone never qualify. Reward history is an append-only earned/reversed/redeemed/restored ledger with derived per-category balances. Ordinary Clients receive one-level internal ServiceCredit accounted for in the organization's Base Currency and redeemable through the existing Finance obligation/payment flow; cross-currency redemption requires configured FX evidence, and ordinary Clients cannot request monetary payout. Partners receive a separate monetary reward category and may request manual payout reservations one currency at a time; CRM may approve, reject, cancel, or mark a request paid after an external payment, with all transitions audited. There is no automated payment-provider payout, expiry, MLM, implicit 1:1 conversion, PartnerCash cross-currency aggregation, or cross-tenant qualification.

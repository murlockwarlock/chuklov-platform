...
<div class="space-y-3 rounded-xl border border-gray-200 p-4 dark:border-white/10">
    <div class="text-sm font-medium text-gray-950 dark:text-white">{{ __('Текущее медиа') }}</div>
            ...
            <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-white/10">
                ...
                @elseif ($item['type'] === 'document')
                    <div class="flex h-36 items-center justify-center bg-gray-50 px-3 text-center dark:bg-white/5">
                        <div>
                            <div class="text-xs font-bold uppercase text-gray-500 dark:text-gray-400">{{ __('Файл') }}</div>
                            <div class="mt-1 break-all text-sm text-gray-950 dark:text-white">{{ $item['name'] ?: __('Документ') }}</div>
                        </div>
                    </div>
                @else
                ...
                <div class="border-t border-gray-200 px-3 py-2 text-xs text-gray-500 dark:border-white/10 dark:text-gray-400">
                    {{ match ($item['type']) { 'video' => __('Видео'), 'document' => __('Документ'), default => __('Фото') } }}
                    @if ($item['name'])
                        · {{ $item['name'] }}
                    @endif
                </div>

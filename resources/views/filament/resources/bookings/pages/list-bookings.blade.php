        ...
        <div class="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div class="flex min-w-0 flex-wrap items-center gap-2">
                <div class="inline-flex rounded-lg border border-gray-200 bg-white p-1 dark:border-gray-800 dark:bg-gray-900">
                    <button type="button" wire:click="setViewMode('week')" class="rounded-md px-3 py-1.5 text-sm font-medium {{ $viewMode === 'week' ? 'bg-gray-100 text-gray-950 dark:bg-gray-800 dark:text-white' : 'text-gray-600 dark:text-gray-300' }}">{{ __('Неделя') }}</button>
                    <button type="button" wire:click="setViewMode('list')" class="rounded-md px-3 py-1.5 text-sm font-medium {{ $viewMode === 'list' ? 'bg-gray-100 text-gray-950 dark:bg-gray-800 dark:text-white' : 'text-gray-600 dark:text-gray-300' }}">{{ __('Список') }}</button>
                </div>
                <div class="flex items-center gap-2">
                    <button type="button" wire:click="previousWeek" class="inline-flex size-10 shrink-0 items-center justify-center rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800" aria-label="{{ __('Предыдущая неделя') }}">
                    ...
                    <button type="button" wire:click="nextWeek" class="inline-flex size-10 shrink-0 items-center justify-center rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800" aria-label="{{ __('Следующая неделя') }}">
                        <x-filament::icon icon="heroicon-o-chevron-right" class="size-5" />
                    </button>
                    <button type="button" wire:click="today" class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Сегодня') }}</button>
                </div>
            </div>
            ...
            <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end">
                @if (count($this->specialistOptions) > 1)
                    <div class="w-full sm:w-64">
                        <label for="booking-journal-specialist" class="fi-fo-field-wrp-label inline-flex text-sm font-medium text-gray-950 dark:text-white">{{ __('Специалист') }}</label>
                ...
                <div class="grid min-w-0 grid-cols-[4rem_repeat(7,minmax(0,1fr))] border-b border-gray-200 dark:border-gray-800">
                    ...
                    @foreach ($days as $day)
                        <div class="min-w-0 border-r border-gray-200 px-2 py-3 last:border-r-0 dark:border-gray-800 {{ $day['is_today'] ? 'bg-primary-50 dark:bg-primary-950/30' : '' }}">
                            ...
                            <div class="mt-1 truncate text-center text-[11px] {{ $day['is_working'] ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-400 dark:text-gray-500' }}">{{ $day['is_working'] ? __('Рабочий день') : __('Не работает') }}</div>
                        </div>
                    @endforeach
                </div>
                    ...
                    <section class="min-w-0 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 {{ $day['is_today'] ? 'ring-2 ring-primary-500' : '' }}">
                        <div class="flex min-w-0 items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-xs {{ $day['is_working'] ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-400 dark:text-gray-500' }}">{{ $day['is_working'] ? __('Рабочий день') : __('Не работает') }}</p>
                            </div>
                            @if ($day['is_working'])

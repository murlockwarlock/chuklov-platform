<x-filament-panels::page>
    @php
        ...
        $weekdays = [1 => __('Пн'), 2 => __('Вт'), 3 => __('Ср'), 4 => __('Чт'), 5 => __('Пт'), 6 => __('Сб'), 7 => __('Вс')];
    @endphp

    <div class="flex min-w-0 flex-col gap-6">
        <div class="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0">
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Календарь конкретных дат. Регулярный график задаётся в «Настройках расписания».') }}</p>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $this->specialistScheduleTimezoneLabel() }}</p>
            </div>
            <div class="w-full lg:max-w-xs">
                <label for="work-schedule-specialist" class="fi-fo-field-wrp-label inline-flex text-sm font-medium text-gray-950 dark:text-white">{{ __('Специалист') }}</label>
        ...
        <section class="min-w-0 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <div class="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div class="flex min-w-0 items-center gap-2">
                    <button type="button" wire:click="previousMonth" class="inline-flex size-10 shrink-0 items-center justify-center rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800" aria-label="{{ __('Предыдущий месяц') }}">
                    ...
                    <button type="button" wire:click="nextMonth" class="inline-flex size-10 shrink-0 items-center justify-center rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800" aria-label="{{ __('Следующий месяц') }}">
                        <x-filament::icon icon="heroicon-o-chevron-right" class="size-5" />
                    </button>
                    <button type="button" wire:click="currentMonth" class="shrink-0 rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Текущий месяц') }}</button>
                </div>

                <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Выбрано: :count', ['count' => count($selectedDates)]) }}</span>
                    <button type="button" wire:click="applyPreset('weekdays')" class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Будни') }}</button>
                    <button type="button" wire:click="applyPreset('all')" class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Все дни') }}</button>
                    <button type="button" wire:click="applyPreset('even')" class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Чётные') }}</button>
                    <button type="button" wire:click="applyPreset('odd')" class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Нечётные') }}</button>
                    <button type="button" wire:click="clearDateSelection" @disabled($selectedDates === []) class="rounded-lg border border-rose-200 px-3 py-2 text-sm font-medium text-rose-700 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-rose-900/60 dark:text-rose-300 dark:hover:bg-rose-950/30">{{ __('Снять выбор') }}</button>
                </div>
            </div>

            @if ($this->pendingPresetLabel() !== null)
                <p class="mt-4 rounded-lg bg-primary-50 px-3 py-2 text-sm text-primary-800 dark:bg-primary-950/30 dark:text-primary-200">{{ __(':preset: выбраны даты текущего месяца.', ['preset' => $this->pendingPresetLabel()]) }}</p>
            @endif
                    ...
                    @else
                        ...
                        >
                            ...
                            @if ($this->isToday($cell['date']))
                                <span class="mt-1 inline-flex items-center gap-1 text-[10px] font-medium text-gray-500 dark:text-gray-400" aria-label="{{ __('Сегодня') }}">
                                    <span class="size-1.5 rounded-full bg-gray-400 dark:bg-gray-500"></span>
                                    {{ __('Сегодня') }}
                                </span>
                            @endif
                            @if ($cell['is_working'])
                                <span class="mt-2 block text-[11px] font-medium text-emerald-700 dark:text-emerald-300">{{ __('Работает') }}</span>
        ...
        @if ($selectedDates !== [])
            <section class="min-w-0 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                @if ($errorMessage !== '')
                    <div role="alert" class="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-200">{{ __($errorMessage) }}</div>
                @endif
                ...
                @if ($this->hasPendingImpact())
                    <div class="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-100">
                        <p class="font-medium">{{ __('Изменение затрагивает будущие записи') }}</p>
                        <ul class="mt-2 list-inside list-disc space-y-1">
                            @foreach ($impactBookings as $booking)
                                <li>{{ $booking['client'] ?? __('Клиент') }} · {{ $booking['service'] ?? __('Услуга') }} · {{ \App\Filament\Support\ScheduleImpactPreview::dateTimeLabel($booking['local_start'] ?? null) }}</li>
                            @endforeach
                        </ul>
                        <label class="mt-3 flex min-w-0 items-start gap-2">
                            ...
                            <span>{{ __('Подтверждаю изменение графика. Записи сохранятся и потребуют отдельного решения.') }}</span>
                        </label>
                    </div>
                @endif
                            ...
                            <div class="mt-2 grid min-w-0 gap-2">
                                @forelse ($overrideIntervals as $index => $interval)
                                    <div wire:key="override-interval-{{ $index }}" class="grid min-w-0 grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] items-end gap-2">
                                        ...
                                        <button type="button" wire:click="removeOverrideInterval({{ $index }})" class="inline-flex size-10 shrink-0 items-center justify-center rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800" aria-label="{{ __('Удалить интервал') }}">
                                            <x-filament::icon icon="heroicon-o-trash" class="size-4" />
                                        </button>
                                    </div>
                                @empty
                                    <p class="rounded-lg border border-dashed border-gray-300 px-3 py-2 text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">{{ __('Добавьте хотя бы один рабочий интервал.') }}</p>
                                @endforelse
                            </div>
                        </div>
                    @else

<x-filament-panels::page>
    <div class="space-y-6">
        <div class="rounded-xl border p-5 {{ $clientCompanion['ready'] ? 'border-emerald-200 bg-emerald-50/50 dark:border-emerald-900 dark:bg-emerald-950/20' : ($isAiEnabled ? 'border-amber-200 bg-amber-50/60 dark:border-amber-900 dark:bg-amber-950/20' : 'border-rose-300 bg-rose-50 dark:border-rose-900 dark:bg-rose-950/30') }}">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <div class="flex items-center gap-2">
                        <span class="inline-flex h-3 w-3 rounded-full {{ $clientCompanion['ready'] ? 'bg-emerald-500 animate-pulse' : ($isAiEnabled ? 'bg-amber-500' : 'bg-rose-500') }}"></span>
                    ...
                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">
                        ...
                        @else
                            Ключ провайдера сам по себе не запускает сценарий: нужны активные модель и версия промпта.
                        @endif
                    </p>
                ...
                <div class="flex flex-wrap gap-2">
                    @if($canManageAi && $clientCompanion['status'] === 'needs_setup')

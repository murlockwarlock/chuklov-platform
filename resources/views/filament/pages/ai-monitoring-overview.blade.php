    ...
    <div class="space-y-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-base font-semibold text-slate-900 dark:text-slate-100">
                            @if($clientCompanion['status'] === 'disabled')
                                {{ __('AI временно отключён') }}
                            @elseif($clientCompanion['ready'])
                                {{ __('AI-компаньон готов к работе') }}
                            @elseif($clientCompanion['status'] === 'provider_unavailable')
                                {{ __('AI-компаньон временно недоступен') }}
                            @else
                                {{ __('AI-компаньон требует настройки') }}
                            @endif
                        </h3>
                    </div>
                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">
                        @if(!$isAiEnabled)
                            {{ __('Новые платные AI-запросы временно остановлены для всей организации.') }}
                        @elseif($clientCompanion['status'] === 'disabled')
                            {{ __('Сценарий клиентского компаньона отключён в ограничениях AI.') }}
                        @elseif($clientCompanion['ready'])
                            {{ __('Запросы обрабатываются в соответствии с установленным дневным бюджетом.') }}
                        @elseif($clientCompanion['status'] === 'provider_unavailable')
                            {{ __('Настройка сохранена, но AI сейчас недоступен. Проверьте подключение и повторите попытку.') }}
                        @else
                            {{ __('Завершите настройку AI-компаньона.') }}
                        @endif
                    </p>
                ...
                <div class="flex flex-wrap gap-2">
                    @if($canManageAi && in_array($clientCompanion['status'], ['needs_setup', 'provider_unavailable'], true))
                        <a class="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" href="{{ $clientCompanion['promptUrl'] }}">{{ __('Настроить промпт') }}</a>
                        <a class="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" href="{{ $clientCompanion['providerUrl'] }}">{{ __('Настроить модель') }}</a>
                    @endif
                    <button
                    ...
                    >
                        {{ $isAiEnabled ? __('Отключить AI') : __('Включить AI') }}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                <p class="text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ __('Дневной расход') }}</p>
                <div class="mt-2 flex items-baseline gap-2">
                    <span class="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
                        ${{ $spentToday }}
                    </span>
                    <span class="text-xs text-slate-500">{{ __('из') }} ${{ $maxDailySpend }}</span>
                </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                <p class="text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ __('Зарезервировано') }}</p>
                <div class="mt-2">
                    <span class="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
                        ${{ $reservedToday }}
                    </span>
                </div>
                <p class="mt-2 text-xs text-slate-500">{{ __('Ожидаемые расходы текущих запросов') }}</p>
            </div>

            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                <p class="text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ __('Запусков сегодня') }}</p>
                <div class="mt-2">
                    <span class="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
                        {{ $runsCountToday }}
                    </span>
                </div>
                <p class="mt-2 text-xs text-slate-500">{{ __('Обработано сценариев и запросов') }}</p>
            </div>

            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                <p class="text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ __('Ошибки / Сбои') }}</p>
                <div class="mt-2">
                ...
                <p class="mt-2 text-xs text-slate-500">
                    {{ __(':percent% отказов сегодня', ['percent' => $runsCountToday > 0 ? round(($failedRunsCountToday / $runsCountToday) * 100, 1) : 0]) }}
                </p>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-slate-200 bg-white shadow-xs dark:border-slate-800 dark:bg-slate-900 overflow-hidden">
            <div class="border-b border-slate-200 px-5 py-4 dark:border-slate-800">
                <h4 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ __('Подключённые сервисы AI') }}</h4>
            </div>
                    ...
                    <div class="flex items-center justify-between px-5 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500">{{ __('Моделей: :count', ['count' => $provider->models_count]) }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium {{ $provider->is_enabled ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400' }}">
                                {{ $provider->is_enabled ? __('Включён') : __('Отключён') }}
                            </span>
                        </div>
                    </div>
                @empty
                    <div class="px-5 py-8 text-center text-sm text-slate-500">
                        {{ __('Добавьте подключение AI в разделе «Провайдеры и модели».') }}
                    </div>
                @endforelse
            </div>
        </div>

...
class CreateBooking
{
    ...
    public function handle(
        ...
        $autoGeneratedIdempotencyKey = $idempotencyKey === null;
        ...
        return DB::transaction(function () use (
            ...
            while ($idempotency->booking_id !== null) {
                ...
                $existingBooking = $bookingQuery->firstOrFail();
                if (! $autoGeneratedIdempotencyKey || $existingBooking->status !== BookingStatus::Cancelled) {
                    $idempotency->forceFill(['last_used_at' => now()])->save();

                    return $existingBooking;
                }

                $idempotencyKey = $this->nextAutomaticIdempotencyKey($idempotencyKey, $existingBooking);
                $idempotency = $this->lockIdempotencyKey(
                    organizationId: $organization->getKey(),
                    idempotencyKey: $idempotencyKey,
                    actorType: $actorType,
    ...
    private function nextAutomaticIdempotencyKey(string $idempotencyKey, Booking $booking): string
    {
        return hash('sha256', implode('|', [
            $idempotencyKey,
            'cancelled-booking-retry',
            $booking->getKey(),
            $booking->event_version,
        ]));
    }

...
final readonly class CompanionActionButton
{
    public function __construct(
        ...
        if (count(array_filter([$this->callbackData, $this->url, $this->webAppUrl], static fn (?string $target): bool => $target !== null)) !== 1) {
        ...
        foreach ([$this->url, $this->webAppUrl] as $target) {
            if ($target !== null && (filter_var($target, FILTER_VALIDATE_URL) === false || ! preg_match('/^https:\/\//i', $target))) {

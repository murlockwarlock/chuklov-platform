...
final readonly class AiEvaluationComparison
{
    ...
    public function toRussianSummary(): string
    {
        ...
        foreach ($this->runs as $index => $run) {
            ...
            $lines[] = 'Среднее время: '.self::duration((int) ($run['average_latency_ms'] ?? 0)).'; '.self::costLabel($run['estimated_cost'] ?? [], 'Расчётная стоимость Chuklov', (int) ($run['estimated_cost_unknown_count'] ?? 0)).'; '.self::costLabel($run['provider_cost'] ?? [], 'Стоимость AI-сервиса', (int) ($run['provider_cost_unknown_count'] ?? 0)).'.';

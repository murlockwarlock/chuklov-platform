...
final class StartPendingKnowledgeIngestion
{
    ...
    public function handle(User $actor, KnowledgeSource $source, int $revisionId): KnowledgeRevision
    {
        ...
        $revision = DB::transaction(function () use ($actor, $source, $organization, $revisionId): KnowledgeRevision {
            ...
            if ($lockedSource->status !== KnowledgeSourceStatus::Active) {
                throw ValidationException::withMessages(['source' => 'Источник выключен или недоступен для обработки.']);
            }
            ...
            if (! $latestRevision instanceof KnowledgeRevision || $latestRevision->getKey() !== $lockedRevision->getKey()) {
                throw ValidationException::withMessages(['revision' => 'Запустить можно только обработку последней версии.']);
            }
            ...
            $this->audit->handle(
                ...
                actor: $actor,
                action: 'knowledge.ingestion.pending_start_requested',
        ...
        try {
            ...
            throw ValidationException::withMessages(['revision' => 'Не удалось запустить обработку. Попробуйте ещё раз.']);
        }
